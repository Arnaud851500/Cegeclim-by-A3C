      ...
      <section className="mb-6 rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
        <div className="mb-3 flex items-start justify-between gap-4">
          <div>
            ...
            <p className="text-xs font-bold uppercase text-slate-500">Lecture en {metric === 'ca_ht' ? 'CA HT' : 'quantité pertinente'} · valeur {selectedYear} avec {selectedYear - 1} entre parenthèses · clic sur une cellule mensuelle pour afficher le détail</p>
          </div>
                ...
                <tr key={`${row.famille_macro}-${row.flux}`} className="border-b border-slate-100 odd:bg-slate-50">
                        ...
                        >
                          {value ? (
                            <span className="flex flex-col items-end leading-tight">
                              ...
                              {safeNumber(row.moisN1[month]) ? (
                                <span className={`text-[11px] font-bold ${selected ? 'text-blue-100' : 'text-slate-500'}`}>
                                  ({formatMetric(safeNumber(row.moisN1[month]), metric)})
                                </span>
                              ) : null}
                            </span>
                          ) : safeNumber(row.moisN1[month]) ? (
                            <span className="flex flex-col items-end leading-tight text-slate-400">
                              <span>—</span>
                              <span className="text-[11px] font-bold">({formatMetric(safeNumber(row.moisN1[month]), metric)})</span>
                            </span>
                          ) : '—'}
                        </button>
                      </td>
                  ...
                  <td className="px-3 py-2 text-right font-black">
                    <div className="flex flex-col items-end leading-tight">
                      <span>{formatMetric(row.total, metric)}</span>
                      {safeNumber(row.totalN1) ? (
                        <span className="text-[11px] font-bold text-slate-500">({formatMetric(row.totalN1, metric)})</span>
                      ) : null}
                    </div>
                  </td>
                </tr>
