...
export default function PortefeuilleLivraisonPage() {
  ...
  useEffect(() => {
    ...
    const requestedControl = String(params.get('controle') || '').trim().toLowerCase()
    if (requestedControl !== 'frais-port-manquant') return
  ...
  const documents = useMemo<DocumentPortefeuille[]>(() => {
    const controlByKey = new Map(controlesFraisPort.map((row) => [controlDocKey(row), row]))
    const map = new Map<
      string,
  ...
  const expeditions = useMemo(() =>
    Array.from(new Set(documents.map((doc) => doc.expedition).filter(Boolean))).sort((a, b) => a.localeCompare(b, 'fr')),
  [documents])

  const depotsEntete = useMemo(() =>
    Array.from(new Set(documents.map((doc) => doc.depot_entete).filter(Boolean))).sort((a, b) => a.localeCompare(b, 'fr')),
  [documents])
  ...
  const documentsFiltresControle = useMemo(() => {
    ...
    return documents.filter((doc) => {
      ...
      if (selectedControle === 'ANOMALIES' && ['OK', 'PL_A_CONFIRMER', 'NON_APPLICABLE'].includes(doc.statut_controle)) return false
