...
// Partie interactive de la page publique /installer (25/09/2026) — cible du
// QR code de lancement du Compagnon auprès des commerciaux. Le fichier
// page.tsx voisin (serveur) pose le manifest personnalisé puis rend ce
// composant.
//
...
export default function InstallerClient() {
  ...
  useEffect(() => {
    ...
    // Filet de sécurité : page.tsx pose déjà le manifest personnalisé côté
    // serveur ; on s'assure ici qu'un seul lien manifest subsiste et qu'il
    // pointe bien vers la version qui porte l'identifiant (start_url).
    if (u) {
      const href = `/api/manifest?u=${encodeURIComponent(u)}`
      const liens = Array.from(document.querySelectorAll<HTMLLinkElement>('link[rel="manifest"]'))
      liens.forEach((lien, i) => {
        if (i === 0) lien.setAttribute('href', href)
        else lien.remove()
      })
      if (liens.length === 0) {
