...
/**
 * Habillage d'une page de login : ce que voit la personne avant de savoir
 * qui se connecte. C'est ce paramétrage qui permet à /financement/login
 * de se présenter comme "Le pilotage des aides financières CEGECLIM" sans
 * dupliquer toute la mécanique d'authentification ci-dessous.
 */
export type LoginBrand = {
  /** Petit libellé en majuscules au-dessus du titre (ex: "Suivi commercial & prospect"). */
  eyebrow: string
  /** Titre principal du volet gauche (ex: "Le pilotage commercial CEGECLIM"). */
  title: string
  /** Paragraphe sous le titre. */
  subtitle: string
  /** Début de la signature en bas du volet (ex: "Distributeur de solutions"). */
  signaturePrefix: string
  /** Mot accentué de la signature (ex: "durables"). */
  signatureAccent: string
  /** Les 3 chiffres-clés affichés dans le rail (masqué sur petit écran). */
  railItems: Array<{ label: string; value: string }>
  /** Page vers laquelle rediriger si l'utilisateur n'a pas de default_landing_page en base. */
  defaultLandingPage: string
  /** Phrase d'aide sous le champ email. */
  formHelper: string
  /** Affiche ou non les annonces temporaires internes (app_announcements). */
  showAnnonce?: boolean
}

async function getUserLandingPage(email: string | null | undefined, fallback: string) {
...
export default function LoginScreen({ brand }: { brand: LoginBrand }) {
  ...
  useEffect(() => {
    ...
    if (sessionEmail) {
      void (async () => {
        const landingPage = await getUserLandingPage(sessionEmail, brand.defaultLandingPage)
  ...
  const handleLogin = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      try {
        landingPage = await getUserLandingPage(data.user?.email || normalizedEmail, brand.defaultLandingPage)
      } catch (err) {
        console.warn('[login] page d\'accueil du profil illisible, repli sur', brand.defaultLandingPage, err)
      }
  ...
  return (
    <main
      ...
      style={styles.page}
    >
      {brand.showAnnonce !== false && <AnnonceTemporaire />}
      ...
      >
        ...
        <div style={styles.signature} className="loginSignature">
          {brand.signaturePrefix} <span style={styles.signatureAccent}>{brand.signatureAccent}</span>
        </div>

        <div style={styles.visuelBas} className="loginVisuelBas">
          <div style={styles.eyebrow} className="loginEyebrow">{brand.eyebrow}</div>
          <h1 style={styles.titre} className="loginTitre">{brand.title}</h1>
          <p style={styles.sousTitre} className="loginSousTitre">
            {brand.subtitle}
          </p>

          <div style={styles.rail} className="loginRail">
            {brand.railItems.map((item) => (
              <div key={item.label}>
                <div style={styles.railEtiquette}>{item.label}</div>
                <div style={styles.railValeur}>{item.value}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section style={styles.voletForm} className="loginVolet">
        <div style={styles.carte} className="loginCarte">
          ...
          <div style={styles.formAide} className="loginFormAide">{brand.formHelper}</div>
      ...
      <style jsx global>{`
        ...
        @media (max-width: 600px) and (max-height: 640px) {
          .loginVisuel { height: 34dvh; padding: 14px 20px 14px !important; gap: 10px; }
